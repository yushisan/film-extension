...
define(function(require, exports, module) {
    ...
    SQL['theater'] = {
        ...
        'create': 'CREATE TABLE IF NOT EXISTS {0} \
                    (id integer primary key autoincrement, \
                     cid,title,year,url,pic,dir,actor,brief,checkuptime,douban,newscore,status,flag,read,timestamp)',
        //向表中插入数据
        ...
        //得到未读消息
        'unread': 'select count(*) as unread_count from {0} where read = 0',
        //设置消息为已读
        'update_read': 'update {0} set read = {1} where id = {2}',
        //设置所有消息为已读
        'update_read_all': 'update {0} set read = 1',
        //查询表中的数据
    ...
    SQL['trailer'] = {
        ...
        'create': 'CREATE TABLE IF NOT EXISTS {0} \
                    (id integer primary key autoincrement, \
                     cid,title,year,url,pic,dir,actor,brief,checkuptime,douban,newscore,read,\
                    timestamp)',
        'check':'select count(id) as count from {0}  where cid = "{1}"',
        'insert': 'insert into {0} \
                (cid,title,year,url,pic,dir,actor,brief,checkuptime,douban,newscore,read,timestamp) \
                values \
    ...
    SQL['activity'] = {
        ...
        'create': 'CREATE TABLE IF NOT EXISTS {0} \
                    (id integer primary key autoincrement, \
                    aid,title,pic,url,desc,level,start,end,type,flag,read,timestamp)',
        //向表中插入数据
        'insert': 'insert into {0} \
                (aid,title,pic,url,desc,level,start,end,type,flag,read,timestamp) \
                values \
